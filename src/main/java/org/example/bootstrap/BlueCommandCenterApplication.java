...
public class BlueCommandCenterApplication {

    private static final int PORT = 5000;
    private static final int KILL_LISTEN_PORT = 6000;
    private static final int REMOTE_CC_PORT = 6001;
    ...
    private static final String SIDE_LABEL = "BLUE";
    private static final Side MY_SIDE = Side.BLUE;
    ...
    public static void main(String[] args) {
        ...
        try (ServerSocket squadronServerSocket = new ServerSocket(PORT);
            ...
            System.out.println(AnsiColors.cyan("Blue Command Center listening on port " + PORT));
            System.out.println(AnsiColors.cyan("Blue Command Center listening for kill notifications on port " + KILL_LISTEN_PORT));

            Thread remoteConnectThread = new Thread(() -> connectToRemoteCc(), "BLUE-CC-Remote-Connect");
            Thread acceptThread = new Thread(() -> acceptSquadrons(squadronServerSocket, objectMapper), "BLUE-CC-Accept");
            Thread commandThread = new Thread(() -> commandLoop(objectMapper), "BLUE-CC-Command");
            Thread killThread = new Thread(() -> killListenLoop(killServerSocket, objectMapper), "BLUE-CC-Kill-Listen");
    ...
    private static void connectToRemoteCc() {
        while (remoteCcWriter == null) {
            try {
                ...
                System.out.println("[BLUE CC] Connected to RED Command Center on port " + REMOTE_CC_PORT);
            } catch (Exception e) {
                System.out.println("[BLUE CC] Waiting for RED Command Center on port " + REMOTE_CC_PORT + "...");
                try {
                    Thread.sleep(1500);
    ...
    private static void acceptSquadrons(ServerSocket serverSocket, ObjectMapper objectMapper) {
        try {
            while (true) {
                ...
                System.out.println("[BLUE CC] Squadron connected from " + squadronSocket.getRemoteSocketAddress());
                ...
                Thread receiveThread = new Thread(
                        ...
                        "BLUE-CC-Receive-" + squadronSocket.getPort()
                );
                receiveThread.start();
            }
        } catch (Exception e) {
            System.out.println("[BLUE CC] Accept thread ended: " + e.getMessage());
        }
    }
    ...
    private static void commandLoop(ObjectMapper objectMapper) {
        ...
        System.out.println("Enter commands for BLUE side:");
        System.out.println("  SHOW");
        System.out.println("  P1 RETURN");
        System.out.println("  P2 PATROL 0.0 2.0 0.0 2.0");
        System.out.println("  P3 PATROL A2 B3");
        System.out.println("  P1 ATTACK C3");
        ...
        try {
            while (true) {
                ...
                if (squadronWriters.isEmpty()) {
                    System.out.println("[BLUE CC] No connected squadrons.");
                    continue;
                }

                String[] parts = line.trim().split("\\s+");
                if (parts.length < 2) {
                    System.out.println("[BLUE CC] Invalid command format.");
                    continue;
                }
            ...
            System.out.println("[BLUE CC] Command thread ended: " + e.getMessage());
        }
    }
    ...
    private static void killListenLoop(ServerSocket killServerSocket, ObjectMapper objectMapper) {
        try {
            while (true) {
                ...
                System.out.println("[BLUE CC] Kill-link connected from " + socket.getRemoteSocketAddress());
                ...
                Thread t = new Thread(() -> {
                    try (BufferedReader reader = new BufferedReader(new InputStreamReader(socket.getInputStream()))) {
                        ...
                        System.out.println("[BLUE CC] Kill-link reader ended: " + e.getMessage());
                    }
                }, "BLUE-CC-Kill-Reader-" + socket.getPort());

                t.start();
            }
        } catch (Exception e) {
            System.out.println("[BLUE CC] Kill listen thread ended: " + e.getMessage());
        }
    }
    ...
    private static void receiveLoop(BufferedReader reader, PrintWriter writer, ObjectMapper objectMapper) {
        try {
            ...
            System.out.println("[BLUE CC] Receive thread ended: " + e.getMessage());
        } finally {
    ...
    private static void handleKillNotification(KillNotification notification, ObjectMapper objectMapper) {
        ...
        if (LOG_CC_KILLS) {
            System.out.printf("[BLUE CC] KILL CONFIRMED by remote CC: %s destroyed%n",
        ...
        try {
            ...
            if (LOG_CC_KILLS) {
                System.out.printf("[BLUE CC] Forwarded DESTROY command for %s to BLUE squadron%n",
                        colorizeAircraftId(victimId));
            }
        } catch (Exception e) {
            System.out.println("[BLUE CC] Failed to forward DESTROY command: " + e.getMessage());
        }
    }
...
//            System.out.printf("[BLUE CC] missile %s LAUNCHED by %s towards %s from [%s] lastKnownTarget=[%s]%n",
...
//        }
        ...
        if (message.isHit()) {
            if (LOG_CC_MISSILES) {
                System.out.printf("[BLUE CC] missile %s HIT target %s at [%s]%n",
                        message.getMissileId(),
                        colorizeAircraftId(message.getTargetId()),
                        BoardUtils.positionToCell(message.getPosition()));
            }
        ...
        if (message.isSelfDestructed()) {
            if (LOG_CC_MISSILES) {
                System.out.printf("[BLUE CC] missile %s SELF_DESTRUCTED while tracking %s at [%s]%n",
                        message.getMissileId(),
                        colorizeAircraftId(message.getTargetId()),
                        BoardUtils.positionToCell(message.getPosition()));
            }
    ...
    private static void sendKillNotificationToRemote(String targetId) {
        if (remoteCcWriter == null) {
            System.out.println("[BLUE CC] No link to RED Command Center, cannot send kill notification for " + targetId);
            return;
        }
        ...
        try {
            ...
            System.out.println("[BLUE CC] Sent kill notification for " + targetId + " to RED Command Center.");
        } catch (Exception e) {
            System.out.println("[BLUE CC] Failed to send kill notification: " + e.getMessage());
        }
    }
    ...
    private static void handleAircraftReport(AircraftReportMessage message) {
        ...
        if (LOG_CC_FRIENDLY_MOVES) {
            if (oldCell == null) {
                System.out.printf("[BLUE CC] %s registered at [%s]%n",
                        colorizeAircraftId(aircraftId), newCell);
            } else if (!newCell.equals(oldCell)) {
                System.out.printf("[BLUE CC] %s moved %s -> %s%n",
    ...
    private static void updateEnemyContacts(List<RadarContact> contacts) {
        ...
        for (RadarContact contact : contacts) {
            ...
            if (LOG_CC_ENEMY_MOVES) {
                if (oldCell == null) {
                    System.out.printf("[BLUE CC] enemy %s detected at [%s]%n",
                            colorizeAircraftId(contact.getId()), newCell);
                } else if (!newCell.equals(oldCell)) {
                    System.out.printf("[BLUE CC] enemy %s moved %s -> %s%n",
    ...
    private static CommandMessage buildCommand(String aircraftId, String cmd, String[] parts) {
        if ("RETURN".equals(cmd)) {
            System.out.println("[BLUE CC] Sending RETURN to " + aircraftId);
            return new CommandMessage(CommandMessage.CommandType.RETURN_TO_BASE, aircraftId);
        }

        if ("ATTACK".equals(cmd)) {
            if (parts.length != 3) {
                System.out.println("[BLUE CC] Invalid ATTACK command.");
            ...
            if (targetPos == null) {
                System.out.println("[BLUE CC] Unknown or undetected enemy target: " + targetId);
                return null;
            }
            ...
            System.out.printf("[BLUE CC] Sending ATTACK to %s -> %s at (%.1f, %.1f)%n",
        ...
        if ("PATROL".equals(cmd)) {
            if (parts.length == 4) {
                ...
                System.out.printf("[BLUE CC] Sending PATROL to %s for box %s -> %s%n",
            ...
            if (parts.length == 6) {
                ...
                System.out.printf("[BLUE CC] Sending PATROL to %s for x[%.1f, %.1f], y[%.1f, %.1f]%n",
            ...
            System.out.println("[BLUE CC] Invalid PATROL command.");
            System.out.println("Use:");
            System.out.println("  P1 PATROL A2 B3");
            System.out.println("  P1 PATROL 0.0 2.0 0.0 2.0");
            return null;
        }

        System.out.println("[BLUE CC] Unknown command: " + cmd);
        return null;
    }
